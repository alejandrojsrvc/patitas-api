...
export const extractAttribute = (html: string, tag: string, attribute: string): string[] => {
  const values: string[] = [];
...
export const extractMeta = (html: string, property: string): string | null => {
  const pattern = new RegExp(`<meta\\b[^>]*(?:property|name)\\s*=\\s*["']${property}["'][^>]*>`, 'i');
...
export const extractSectionText = (html: string, heading: string): string | null => {
  const headingPattern = new RegExp(`<h[1-6]\\b[^>]*>\\s*${escapeRegExp(heading)}\\s*<\\/h[1-6]>([\\s\\S]*?)(?=<h[1-6]\\b|<footer\\b|$)`, 'i');
