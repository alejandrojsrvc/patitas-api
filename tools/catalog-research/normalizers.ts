...
export const normalizeMoney = (value: string | number | null | undefined): number | null => {
...
export const normalizeWeightGrams = (value: string | number | null | undefined): number | null => {
  if (typeof value === 'number') return value > 0 ? Math.round(value) : null;
  if (!value) return null;
  const match = value.replace(',', '.').match(/(\d+(?:\.\d+)?)\s*(kg|kilos?|g|gr)/i);
  if (!match) return null;
  const amount = Number(match[1]);
  if (!Number.isFinite(amount) || amount <= 0) return null;
  return Math.round(match[2].toLowerCase().startsWith('k') ? amount * 1000 : amount);
};

export const normalizeAttribute = (value: string | null | undefined): string | null => {
...
export const parseWeightRange = (
  ...
  const open = value.match(/(\d+(?:[.,]\d+)?)\s*(?:kg|kilos?)?\.?\s*(?:o más|en adelante|\+)/i);
  ...
  return single ? { min: Number(single[1].replace(',', '.')), max: null } : null;
};

export const parseDailyRange = (value: string): { min: number; max: number | null } | null => {
  const open = value.match(/(\d+(?:[.,]\d+)?)\s*(?:g|gr|gramos?)?\.?\s*(?:o más|en adelante|\+)/i);
  if (open) return { min: Number(open[1].replace(',', '.')), max: null };
  const range = value.match(/(\d+(?:[.,]\d+)?)\s*[-a]\s*(\d+(?:[.,]\d+)?)/i);
  if (range) {
    return {
...
export const parseFeedingRows = (rows: string[]): FeedingGuideEntry[] => {
  ...
  for (const row of rows) {
    const [rawWeight, rawDailyAmount] = row.split('|').map((item) => item.trim());
...
export const parseFeedingTables = (tables: string[][]): FeedingGuideEntry[] => {
  const simpleRows = tables.flat().filter((row) => /^\s*\d+(?:[.,]\d+)?\s*(?:-\s*\d+(?:[.,]\d+)?\s*)?(?:kg|kilos?)/i.test(row));
  const entries = parseFeedingRows(simpleRows);
  for (const table of tables) {
    const standardHeaderIndex = table.findIndex((row) => {
      const cells = row.split('|').map((cell) => cell.trim());
      return cells.length === 2 && /peso|edad|semana|meses?/i.test(cells[0]) && /gramos?|g\b/i.test(cells[1]);
    });
    if (standardHeaderIndex >= 0) {
      const headerCells = table[standardHeaderIndex].split('|').map((cell) => cell.trim());
      for (const row of table.slice(standardHeaderIndex + 1)) {
        ...
        entries.push({
          ...
          petWeightKgMax: weight.max,
          lifeStage: null,
          conditions: /edad|semana|meses?/i.test(headerCells[0]) ? { age: cells[0] } : {},
          dailyGramsMin: daily.min,
          dailyGramsMax: daily.max,
          rawWeight: cells[0],
    ...
    const headerIndex = table.findIndex((row) => {
      const cells = row.split('|').map((cell) => cell.trim());
      return cells.length > 2 && !/^\d/.test(cells[0]) && /peso|actividad/i.test(cells[0]) && cells.slice(1).some((cell) => parseWeightRange(cell));
    });
    ...
    (entry, index, values) =>
      values.findIndex(
        (candidate) =>
          ...
          JSON.stringify(candidate.conditions) === JSON.stringify(entry.conditions),
      ) === index,
  );
};
...
export const parseCompositionRows = (rows: string[]): CompositionEntry[] => {
  ...
  for (const row of rows) {
    ...
    const numbers = values.map(parsePercentOrNumber).filter((item): item is number => item !== null);
