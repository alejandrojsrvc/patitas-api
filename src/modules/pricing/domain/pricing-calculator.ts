...
import type { PricingCalculation, PricingContext, PricingRuleValues } from './pricing.types';

export class PricingCalculator {
  public calculate(context: PricingContext, rules: PricingRuleValues, options: { fixedCostPerUnit?: string } = {}): PricingCalculation {
    ...
    const fixedMonthlyAllocation = Money.parse(options.fixedCostPerUnit ?? '0.00');
    ...
    const paymentFeeVatRate = paymentFeeVatApplies ? percentToBasisPoints(rules.paymentFeeVatPercent ?? '0.00') : 0;
    const paymentFeeTaxRate = Math.ceil((paymentRate * paymentFeeVatRate) / 10_000);
    ...
    if (totalRate >= 10_000) {
      throw new PricingPreconditionError('La suma de tasas variables y margen debe ser menor que 100%.');
    }

    const fixed = [product, fulfillment, packaging, paymentFixed, shipping, other, fixedMonthlyAllocation].reduce(
      (sum, value) => sum.add(value),
      Money.zero(),
    );
    const recommended = Money.fromCents(divideCeil(fixed.cents * 10_000n, BigInt(10_000 - totalRate)));
    ...
    const resultingMargin = commercial.cents === 0n ? 0 : Number((profit.cents * 1_000_000n) / commercial.cents) / 10_000;
...
const divideCeil = (numerator: bigint, denominator: bigint) => (numerator + denominator - 1n) / denominator;
...
function assertComplete(rules: PricingRuleValues): asserts rules is CompletePricingRules {
  ...
  if (missing.length) {
    throw new PricingPreconditionError(`La configuración de pricing debe estar completa antes de calcular. Faltan: ${missing.join(', ')}.`);
  }
}
