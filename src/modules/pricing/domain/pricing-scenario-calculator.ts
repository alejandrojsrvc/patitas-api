...
export class PricingScenarioCalculator {
  public calculate(input: {
    ...
    if (!input.variants.length) {
      throw new PricingPreconditionError('No hay variantes con precio y costo para analizar el escenario.');
    }
    ...
    const averageSalePrice = average(input.variants.map((item) => item.salePrice));
    const averageProductCost = average(input.variants.map((item) => item.unitCost));
    const fixedMonthly = sumCosts(operatingCosts, 'FIXED_MONTHLY');
    const perOrder =
      sumCosts(operatingCosts, 'PER_ORDER') + money(rules.packagingCost) + money(rules.subsidizedShippingCost) + money(rules.paymentFixedCost);
    const perUnit = sumCosts(operatingCosts, 'PER_UNIT') + money(rules.fulfillmentCost) + money(rules.otherCost);
    const paymentFeeVatApplies = rules.paymentFeeVatApplies !== false;
    const percentRate =
      percent(rules.paymentFeePercent) * (paymentFeeVatApplies ? 1 + percent(rules.paymentFeeVatPercent) / 100 : 1) +
      percent(rules.taxPercent) +
      sumPercentCosts(operatingCosts);
    const averageVariableCost = averageProductCost + perUnit + (averageSalePrice * percentRate) / 100;
    ...
    const breakEvenOrders = contributionPerOrder > 0 ? Math.ceil(fixedMonthly / contributionPerOrder) : null;
    ...
    return {
      ...
      sourceResolved:
        scenario.ordersSource === 'PREVIOUS_PERIOD' && input.previousPeriodOrders !== null
          ? 'PREVIOUS_PERIOD'
      ...
      breakEvenOrders,
      breakEvenRevenue: breakEvenOrders === null ? null : format(averageSalePrice * itemsPerOrder * breakEvenOrders),
      catalogCoverage: {
        variantsConsidered: input.variantsConsidered ?? input.variants.length,
        variantsIncluded: input.variants.length,
        variantsWithoutActiveOffer: Math.max(0, (input.variantsConsidered ?? input.variants.length) - input.variants.length),
        supplierOfferSelection: 'LOWEST_ACTIVE_UNIT_COST',
        inventoryUsed: false,
        productStatusFilterApplied: false,
        includedVariants: input.variants.flatMap((variant) => (variant.detail ? [variant.detail] : [])),
      },
...
const resolveOrders = (scenario: PricingScenario, previousPeriodOrders: number | null): number =>
  scenario.ordersSource === 'PREVIOUS_PERIOD' && previousPeriodOrders !== null ? previousPeriodOrders : scenario.projectedOrders;
...
const average = (values: string[]): number => values.reduce((sum, value) => sum + Number(value), 0) / values.length;

const sumCosts = (costs: OperatingCost[], type: OperatingCost['type']): number =>
  costs.filter((cost) => cost.type === type).reduce((sum, cost) => sum + money(cost.amount), 0);

const sumPercentCosts = (costs: OperatingCost[]): number =>
  costs.filter((cost) => cost.type === 'PERCENT_OF_SALE').reduce((sum, cost) => sum + percent(cost.percent), 0);

const costLines = (costs: OperatingCost[], type: OperatingCost['type']): PricingScenarioCostLine[] =>
  costs
...
const buildCostBreakdown = (input: {
  ...
  const paymentFeeEffectivePercent = paymentFeePercent * (1 + paymentFeeVatPercent / 100);
  ...
  return {
    ...
    rules: {
      ...
      paymentFeeEffectivePercent: formatPercent(paymentFeeVatApplies ? paymentFeeEffectivePercent : paymentFeePercent),
    ...
    averages: {
      ...
      operatingCostPerUnit: format(configuredPerUnit + money(rules.fulfillmentCost) + money(rules.otherCost)),
