...
export const calculateFoodDuration = (input: FeedingCalculationInput, guide: FeedingGuide | null): FeedingCalculationResult => {
  ...
  if (manufacturer) {
    return buildResult({
      ...
      assumptions: ['Estimación basada en la tabla de raciones declarada por el fabricante.'],
    });
  }
  ...
  return buildResult({
    ...
    presentationGrams: input.presentationGrams,
    assumptions: [
      guide ? 'La tabla del fabricante no cubre todos los datos ingresados.' : 'Este producto todavía no tiene una tabla del fabricante cargada.',
...
const resolveManufacturerEntry = (input: FeedingCalculationInput, guide: FeedingGuide): FeedingGuideEntry | null => {
  ...
  const exact = entries.find(
    (entry) => entry.petWeightKgMin === input.petWeightKg && (entry.petWeightKgMax === null || entry.petWeightKgMax >= input.petWeightKg),
  );
  if (exact) return exact;
  const upper = entries.find((entry) => entry.petWeightKgMin > input.petWeightKg);
  const covering = entries.find(
    (entry) => entry.petWeightKgMin < input.petWeightKg && entry.petWeightKgMax !== null && entry.petWeightKgMax >= input.petWeightKg,
  );
  if (covering) return covering;
  const lower = [...entries].reverse().find((entry) => entry.petWeightKgMin < input.petWeightKg);
  if (!upper && lower?.petWeightKgMax === null) return lower;
  if (!lower || !upper) return null;

  const ratio = (input.petWeightKg - lower.petWeightKgMin) / (upper.petWeightKgMin - lower.petWeightKgMin);
  return {
    ...lower,
    ...
    dailyGramsMax: lower.dailyGramsMax !== null && upper.dailyGramsMax !== null ? interpolate(lower.dailyGramsMax, upper.dailyGramsMax, ratio) : null,
  };
};
...
const hasRequiredDimensions = (values: Record<string, string>, dimensions: Record<string, string[]>) =>
  Object.entries(dimensions).every(([key, allowed]) => typeof values[key] === 'string' && (allowed.length === 0 || allowed.includes(values[key])));

const conditionsMatch = (conditions: Record<string, string>, values: Record<string, string>) =>
