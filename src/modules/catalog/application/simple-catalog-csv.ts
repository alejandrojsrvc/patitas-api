...
export const parseSimpleCatalogCsv = (data: Uint8Array): SimpleCatalogCsvRow[] => {
  ...
  if (missing.length > 0) {
    throw new CatalogValidationError(`Faltan columnas obligatorias: ${missing.join(', ')}.`);
  }
  ...
  return rows
    ...
    .map((values, index) => {
      if (values.length !== headers.length) {
        throw new CatalogValidationError(`La fila ${index + 2} tiene ${values.length} columnas; se esperaban ${headers.length}.`);
      }
      ...
      if (seenProductWeights.has(productWeight)) {
        throw new CatalogValidationError(`El producto ${parsed.slug} repite la presentación ${parsed.weightGrams}.`);
      }
...
const mapRow = (row: CsvRecord, rowNumber: number): SimpleCatalogCsvRow => {
  ...
  const weightKg = requiredWeight(row.weight_kg, rowNumber);
  ...
  const supplierUnitCost = optionalSupplierCost(row.cost_price || row.unit_cost, rowNumber);
  if ((supplierName || supplierSku || supplierUnitCost) && (!supplierName || !supplierUnitCost)) {
    throw new CatalogValidationError(`La fila ${rowNumber} debe tener supplier y cost_price para importar la oferta.`);
  }
  const supplierStockStatus = parseSupplierStockStatus(row.supplier_stock_status || row.stock_status, rowNumber);
  return {
    rowNumber,
    sku: row.sku?.trim().toUpperCase() || generateSku(row.brand, row.name, weightGrams),
    barcode,
    name: required('name'),
...
const requiredWeight = (value: string | undefined, rowNumber: number): number => {
  if (!value?.trim()) throw new CatalogValidationError(`La fila ${rowNumber} no tiene weight_kg; el peso logístico es obligatorio.`);
...
const optionalSupplierCost = (value: string | undefined, rowNumber: number): string | null => {
  if (!value?.trim()) return null;
  const normalized = value.replace(',', '.').trim();
  if (!/^\d+(\.\d{1,2})?$/.test(normalized) || Number(normalized) <= 0) {
    throw new CatalogValidationError(`La fila ${rowNumber} tiene cost_price inválido.`);
  }
  return Number(normalized).toFixed(2);
};

const parseSupplierStockStatus = (value: string | undefined, rowNumber: number): SupplierStockStatus => {
  const normalized = value?.trim().toUpperCase() || 'AVAILABLE';
  if (!['AVAILABLE', 'OUT_OF_STOCK', 'ON_REQUEST', 'UNKNOWN'].includes(normalized)) {
    throw new CatalogValidationError(`La fila ${rowNumber} tiene supplier_stock_status inválido.`);
  }
  return normalized as SupplierStockStatus;
};

const optionalNumber = (value: string | undefined, rowNumber: number, field: string): string | null => {
  if (!value?.trim()) return null;
...
const optionalInteger = (value: string | undefined, rowNumber: number, field: string): number | null => {
  if (!value?.trim()) return null;
...
const inferSpecies = (name: string): string => (/gato|gatito|cat/i.test(name) ? 'gato' : 'perro');

const generateSku = (brand: string, name: string, weightGrams: number | null): string => {
