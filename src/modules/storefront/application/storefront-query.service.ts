...
import { emptyCartSummary, toAuthenticatedViewer, toLocationSummary } from './storefront-shell';
...
export class StorefrontQueryService {
  ...
  public async bootstrap(input: { user?: { userId: string; email: string; role: UserRole }; cartToken?: string }) {
    return (await this.resolve(input)).shell;
  }

  public async cartScreen(input: { user?: { userId: string; email: string; role: UserRole }; cartToken?: string }) {
  ...
  private async resolve(
    ...
    const customer = input.user
      ...
      : null;
    const owner = customer ? { customerId: customer.id } : input.cartToken ? { tokenHash: hashAnonymousToken(input.cartToken) } : {};
    const [addresses, cart] = await Promise.all([
      customer ? this.addresses.listForUserByCustomerId(customer.id) : Promise.resolve([]),
      customer || input.cartToken ? (includeFullCart ? this.carts.findActive(owner) : this.carts.findActiveSummary(owner)) : Promise.resolve(null),
    ]);
    ...
    const cartSummary = cart
      ? includeFullCart
        ? {
            id: cart.id,
            itemCount: 'items' in cart ? cart.items.reduce((total, item) => total + item.quantity, 0) : cart.itemCount,
