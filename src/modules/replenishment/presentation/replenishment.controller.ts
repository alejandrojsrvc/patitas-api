...
export class ReplenishmentController {
  ...
  @Get()
  public async list(@Req() request: Request, @Headers('x-order-token') token?: string) {
    return this.plans.list(await owner(request, this.customers, token));
  }
  @Get(':id')
  public async find(@Req() request: Request, @Param('id') id: string, @Headers('x-order-token') token?: string) {
    return this.plans.find(id, await owner(request, this.customers, token));
  }
  @Post()
  public async create(@Req() request: Request, @Body() input: CreateReplenishmentPlanDto, @Headers('x-order-token') token?: string) {
    const planOwner = await owner(request, this.customers, token);
    if (!planOwner.customerId && !planOwner.guestTokenHash) throw new ReplenishmentValidationError('Inicia sesión o presenta X-Order-Token.');
    return this.plans.create({ ...input, guestAccessTokenHash: planOwner.guestTokenHash }, planOwner);
  }
  @Patch(':id/status')
  public async status(
    @Req() request: Request,
    ...
    return this.plans.setStatus(id, await owner(request, this.customers, token), input.status);
  }
  @Post(':id/reorder-cart')
  public async reorder(@Req() request: Request, @Param('id') id: string, @Headers('x-order-token') token?: string) {
