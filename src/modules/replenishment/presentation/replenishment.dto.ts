...
export class CreateReplenishmentPlanDto {
  @ApiPropertyOptional({ format: 'uuid' }) @IsOptional() @IsUUID() public orderId?: string | null;
  ...
  @ApiPropertyOptional() @IsOptional() @IsString() @MaxLength(80) public petBreed?: string | null;
  ...
  @ApiProperty() @Type(() => Date) @IsDate() public estimatedDepletionDate!: Date;
  @ApiProperty({ enum: ['EMAIL', 'WHATSAPP'] }) @IsIn(['EMAIL', 'WHATSAPP']) public channel!: 'EMAIL' | 'WHATSAPP';
  @ApiProperty() @IsString() @MaxLength(40) public consentVersion!: string;
  @ApiProperty({ description: 'Email o WhatsApp consentido; no se devuelve en el plan.' }) @IsString() @MaxLength(320) public destination!: string;
}

export class UpdateReplenishmentStatusDto {
  @ApiProperty({ enum: ['ACTIVE', 'PAUSED', 'CANCELLED', 'COMPLETED'] }) @IsIn(['ACTIVE', 'PAUSED', 'CANCELLED', 'COMPLETED']) public status!: 'ACTIVE' | 'PAUSED' | 'CANCELLED' | 'COMPLETED';
}
