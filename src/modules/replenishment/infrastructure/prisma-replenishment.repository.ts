...
export class PrismaReplenishmentRepository implements ReplenishmentRepository {
  ...
  public async create(input: CreateReplenishmentPlanInput, owner: ReplenishmentOwner) {
    if (input.idempotencyKey) {
      ...
      if (existing && existing.customerId === owner.customerId) return mapPlan(existing);
    }
    ...
    const plan = await this.prisma.replenishmentPlan.create({
      data: {
        ...
        guestAccessTokenHash: owner.guestTokenHash ?? input.guestAccessTokenHash ?? null,
        ...
        nextReminderAt:
          input.remindersEnabled === false ? null : new Date(input.estimatedDepletionDate.getTime() - (input.leadDays ?? 5) * 24 * 60 * 60 * 1000),
      },
      include: planInclude,
    });
    await this.prisma.communicationConsent.create({
      data: {
        customerId: owner.customerId ?? null,
        guestTokenHash: owner.guestTokenHash ?? input.guestAccessTokenHash ?? null,
        channel: input.channel,
  ...
  public async list(owner: ReplenishmentOwner) {
    if (!owner.customerId && !owner.guestTokenHash) throw new ReplenishmentValidationError('Se requiere autenticación o X-Order-Token.');
  ...
  public async find(id: string, owner: ReplenishmentOwner) {
    ...
    if (!row) throw new ReplenishmentValidationError('El plan no existe o no tienes acceso.');
    return mapPlan(row);
  }
  public async setStatus(id: string, owner: ReplenishmentOwner, status: ReplenishmentPlanStatus) {
  ...
  public async updateSchedule(id: string, owner: ReplenishmentOwner, nextReminderAt: Date) {
    await this.find(id, owner);
    return mapPlan(
      await this.prisma.replenishmentPlan.update({
        where: { id },
  ...
  public async recalibrate(id: string, owner: ReplenishmentOwner, days: number, remainingBucket?: string, observedAt?: Date) {
    await this.find(id, owner);
    const estimatedDepletionDate = new Date(observedAt ?? new Date());
    estimatedDepletionDate.setUTCDate(estimatedDepletionDate.getUTCDate() + days);
  ...
  public async updateMobileState(
    ...
    const hasScheduleChange = input.nextReminderAt !== undefined || input.remindersEnabled !== undefined || input.leadDays !== undefined;
  ...
  public async changeProduct(
    ...
    const estimatedDepletionDate = bagStartedAt ? addDays(bagStartedAt, (await this.find(id, owner)).durationDaysMax) : undefined;
    return mapPlan(
      await this.prisma.replenishmentPlan.update({
        where: { id },
        data: {
  ...
  public async startBag(id: string, owner: ReplenishmentOwner, input: { orderId?: string; orderLineId?: string; startedAt?: Date } = {}) {
    const current = await this.find(id, owner);
    if (input.orderId && current.orderId !== input.orderId) throw new ReplenishmentValidationError('El pedido no corresponde al plan de reposición.');
    if (input.orderLineId) {
      ...
      if (!line) throw new ReplenishmentValidationError('La línea del pedido no corresponde al plan.');
    }
    const bagStartedAt = input.startedAt ?? new Date();
    const estimatedDepletionDate = addDays(bagStartedAt, current.durationDaysMax);
    return mapPlan(
      await this.prisma.replenishmentPlan.update({
        where: { id },
        data: {
          ...
          newBagPending: false,
          nextReminderAt: current.remindersEnabled ? addDays(estimatedDepletionDate, -current.leadDays) : null,
        },
        include: planInclude,
      }),
    );
  ...
  public async createReorderCart(id: string, owner: ReplenishmentOwner, options: { anonymousToken?: boolean } = {}) {
    ...
    if (!plan.variant.active || plan.variant.product.status !== 'ACTIVE')
      throw new ReplenishmentValidationError('La variante ya no está disponible.');
    ...
    const token = owner.customerId && !options.anonymousToken ? null : createAnonymousToken();
...
const mapPlan = (value: PlanRecord): ReplenishmentPlan => ({
  ...
  presentation: value.presentationSnapshot ?? value.variant?.presentation ?? null,
