...
export class EstimateService {
  ...
  public async create(customerId: string | null | undefined, input: CreateEstimateInput, guestTokenHash?: string | null) {
    validate(input);
    ...
    estimatedDepletionDate.setUTCDate(estimatedDepletionDate.getUTCDate() + Math.ceil(result.durationDays.max * remainingFraction));
  ...
  public async findOwned(id: string, customerId: string) {
    ...
    if (!result) throw new EstimateValidationError('La estimación no existe o no tienes acceso.');
    return result;
  }
}

const validate = (input: CreateEstimateInput): void => {
  if (!input.pet.name.trim() || !input.pet.species.trim() || !input.pet.lifeStage.trim())
    throw new EstimateValidationError('Los datos de la mascota son obligatorios.');
  if (!Number.isFinite(input.pet.weightKg) || input.pet.weightKg <= 0)
    throw new EstimateValidationError('El peso de la mascota debe ser mayor que cero.');
  const hasCatalogFood = Boolean(input.food.productId && input.food.variantId);
  const hasCustomFood = Boolean(input.food.custom);
  if (hasCatalogFood === hasCustomFood) throw new EstimateValidationError('Indica un alimento del catálogo o uno personalizado.');
  if (input.food.custom && (!input.food.custom.brand.trim() || !input.food.custom.name.trim() || input.food.custom.weightGrams <= 0))
    throw new EstimateValidationError('Los datos del alimento personalizado no son válidos.');
};

const remainingFractionFor = (bucket: CreateEstimateInput['remainingBucket']): number =>
  ({
    ALMOST_FULL: 0.9,
    MORE_THAN_HALF: 0.7,
    ABOUT_HALF: 0.5,
