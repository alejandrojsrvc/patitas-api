...
import { OrderJobsController } from './presentation/order-jobs.controller';

@Module({
  imports: [PrismaModule, AuthModule, CustomersModule, StorageModule],
  controllers: [AdminOrderController, OrderJobsController],
  providers: [
    { provide: ORDER_REPOSITORY, useClass: PrismaOrderRepository },
    {
      provide: OrderService,
      inject: [ORDER_REPOSITORY, CustomerService, STORAGE_PROVIDER],
      useFactory: (repository: OrderRepository, customers: CustomerService, storage: StorageProvider) =>
