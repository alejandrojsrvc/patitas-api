...
export class OrderService {
  ...
  public async list(filter: OrderFilter) {
    ...
    return {
      ...page,
      items: await Promise.all(page.items.map((order) => this.resolveProofUrls(order))),
    };
  }
  ...
  public async create(input: CreateOrderInput) {
    validateOrder(input);
    const customerId = input.customerId ? await this.resolveActiveCustomer(input.customerId) : undefined;
    return this.resolveProofUrls(await this.repository.create(normalizeOrder({ ...input, customerId })));
  }
  ...
  public async uploadPaymentProof(
    ...
    if (!contentType || !PAYMENT_PROOF_TYPES.has(contentType)) throw new OrderValidationError('El comprobante debe ser PDF, JPEG, PNG o WebP.');
    if (!input.data.byteLength || input.data.byteLength > MAX_PAYMENT_PROOF_BYTES)
      throw new OrderValidationError('El comprobante debe pesar entre 1 byte y 10 MB.');
        ...
        order.payments.map(async (payment) => ({
          ...
          proofUrl:
            payment.proofUrl && !isHttpUrl(payment.proofUrl)
              ? await this.storage.getSignedUrl({ bucket: PAYMENT_PROOF_BUCKET, path: payment.proofUrl }, 3_600)
              : payment.proofUrl,
        })),
      ),
    };
...
const PAYMENT_PROOF_TYPES = new Set(['application/pdf', 'image/jpeg', 'image/png', 'image/webp']);
