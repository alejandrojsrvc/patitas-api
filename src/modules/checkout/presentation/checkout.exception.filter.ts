...
export class CheckoutExceptionFilter implements ExceptionFilter {
  public catch(error: DomainError, host: ArgumentsHost): void {
    const status = error instanceof CheckoutNotFoundError ? HttpStatus.NOT_FOUND : error instanceof CheckoutConflictError ? HttpStatus.CONFLICT : HttpStatus.UNPROCESSABLE_ENTITY;
