...
@Module({
  ...
  providers: [
    { provide: CHECKOUT_REPOSITORY, useClass: PrismaCheckoutRepository },
    {
      ...
      inject: [CHECKOUT_REPOSITORY, STORAGE_PROVIDER, PaymentService, ShippingService],
      useFactory: (repository: CheckoutRepository, storage: StorageProvider, payments: PaymentService, shipping: ShippingService) =>
        new CheckoutService(repository, storage, payments, shipping),
    },
    {
      provide: CheckoutBootstrapService,
      inject: [CheckoutService, CustomerAddressService, PaymentProviderConfigurationService],
      useFactory: (checkout: CheckoutService, addresses: CustomerAddressService, paymentConfigurations: PaymentProviderConfigurationService) =>
        new CheckoutBootstrapService(checkout, addresses, paymentConfigurations),
    },
    {
