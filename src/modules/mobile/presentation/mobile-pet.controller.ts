...
import type { CreatePetInput, UpdatePetInput } from '../../pets/domain/pet.types';
...
export class MobilePetController {
  ...
  public async create(@CurrentUser() user: AuthenticatedUser, @Body() input: MobilePetCreateDto) {
    const customer = await this.customers.findByUserId(user.userId);
    const pet = await this.pets.createForMobile(customer.id, toCreatePetInput(input));
    return toMobilePet(pet);
  }

  @Patch(':petId')
  public async update(@CurrentUser() user: AuthenticatedUser, @Param('petId') petId: string, @Body() input: MobilePetUpdateDto) {
    const customer = await this.customers.findByUserId(user.userId);
    const pet = await this.pets.updateForMobile(petId, customer.id, toPetInput(input));
    return toMobilePet(pet);
  }
}

const toPetInput = (input: MobilePetCreateDto | MobilePetUpdateDto): UpdatePetInput => {
