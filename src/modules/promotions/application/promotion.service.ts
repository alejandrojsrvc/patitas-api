...
export class PromotionService {
  ...
  public createCoupon(input: CouponInput) {
    ...
    return this.ensurePromotion(normalized.promotionId).then(() => this.repository.createCoupon(normalized));
  }
  public async updateCoupon(id: string, input: Partial<CouponInput>) {
    ...
    const merged: CouponInput = {
      ...
      maxRedemptions: input.maxRedemptions !== undefined ? input.maxRedemptions : current.maxRedemptions,
      perCustomerLimit: input.perCustomerLimit !== undefined ? input.perCustomerLimit : current.perCustomerLimit,
    };
    validateCoupon(merged);
    if (merged.maxRedemptions !== null && merged.maxRedemptions !== undefined && current.redemptionCount > merged.maxRedemptions)
      throw new PromotionValidationError('El límite del cupón no puede ser menor a los usos ya registrados.');
    if (merged.promotionId !== current.promotionId) await this.ensurePromotion(merged.promotionId);
...
const validatePromotion = (input: Partial<PromotionInput>) => {
  ...
  if (input.value !== undefined && (!/^\d+(\.\d{1,2})?$/.test(input.value) || Number(input.value) < 0))
    throw new PromotionValidationError('El valor de la promoción no es válido.');
  if (input.type === 'PERCENTAGE' && input.value !== undefined && Number(input.value) > 100)
  ...
  if (
    ...
    (!/^(\d+)(\.\d{1,2})?$/.test(input.minimumSubtotal) || Number(input.minimumSubtotal) < 0)
  )
    throw new PromotionValidationError('El subtotal mínimo de la promoción no es válido.');
  if (input.maxRedemptions !== undefined && input.maxRedemptions !== null && (!Number.isInteger(input.maxRedemptions) || input.maxRedemptions < 1))
    throw new PromotionValidationError('El límite de usos de la promoción no es válido.');
  if (input.priority !== undefined && (!Number.isInteger(input.priority) || input.priority < 0))
    throw new PromotionValidationError('La prioridad de la promoción no es válida.');
  ...
  if (input.kind === 'BUNDLE' && (!input.bundleItems?.length || input.bundleItems.some((item) => !item.variantId || item.quantity < 1)))
    throw new PromotionValidationError('Un combo debe tener variantes y cantidades válidas.');
  if (input.bundleItems && new Set(input.bundleItems.map((item) => item.variantId)).size !== input.bundleItems.length)
    throw new PromotionValidationError('Un combo no puede repetir variantes.');
};

const validateCoupon = (input: Partial<CouponInput>) => {
  if (input.code !== undefined && !/^[A-Z0-9][A-Z0-9_-]{2,79}$/.test(input.code.trim().toUpperCase()))
    throw new PromotionValidationError('El código debe tener entre 3 y 80 caracteres alfanuméricos, guion o guion bajo.');
  ...
  if (input.maxRedemptions !== undefined && input.maxRedemptions !== null && (!Number.isInteger(input.maxRedemptions) || input.maxRedemptions < 1))
    throw new PromotionValidationError('El límite de usos del cupón no es válido.');
  if (
    ...
    throw new PromotionValidationError('El límite por cliente del cupón no es válido.');
  validateDateRange(input.startsAt, input.endsAt);
};

const validateDateRange = (startsAt: Date | null | undefined, endsAt: Date | null | undefined) => {
  if (startsAt && endsAt && startsAt > endsAt) throw new PromotionValidationError('La fecha de inicio no puede ser posterior a la fecha de fin.');
};
