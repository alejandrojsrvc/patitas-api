...
interface PersistenceOffer {
  ...
  unitCost: { toString(): string }; currency: string; stockStatus: string;
  leadTimeHours: number | null; minimumQuantity: number; active: boolean;
...
export class PrismaSupplierRepository implements SupplierRepository {
  ...
  public async listSuppliers(filter: SupplierFilter): Promise<SupplierPage> {
    ...
    const [suppliers, total] = await this.prisma.$transaction([
      this.prisma.supplier.findMany({ where, orderBy: { name: 'asc' }, skip: (filter.page - 1) * filter.perPage, take: filter.perPage }),
      this.prisma.supplier.count({ where }),
    ]);
    return { items: suppliers.map(mapSupplier), page: filter.page, perPage: filter.perPage, total };
  }
  ...
  public async listOffers(filter: { supplierId?: string; variantId?: string; active?: boolean }): Promise<SupplierOffer[]> {
    const offers = await this.prisma.supplierOffer.findMany({
      where: filter, orderBy: { updatedAt: 'desc' },
    });
  ...
  public async createOffer(input: CreateSupplierOfferInput): Promise<SupplierOffer> {
    return this.write(async () => mapOffer(await this.prisma.supplierOffer.create({
      data: { ...input, currency: 'ARS' },
    })));
  }
  public async updateOffer(id: string, input: UpdateSupplierOfferInput): Promise<SupplierOffer> {
    return this.write(async () => mapOffer(await this.prisma.supplierOffer.update({
      where: { id }, data: { ...input, revision: { increment: 1 } },
    })));
  }
...
const mapOffer = (offer: PersistenceOffer): SupplierOffer => ({
  id: offer.id, supplierId: offer.supplierId, variantId: offer.variantId,
  supplierSku: offer.supplierSku, unitCost: offer.unitCost.toString(), currency: 'ARS',
  stockStatus: offer.stockStatus as SupplierOffer['stockStatus'],
  leadTimeHours: offer.leadTimeHours, minimumQuantity: offer.minimumQuantity,
  active: offer.active, revision: offer.revision, updatedAt: offer.updatedAt,
});
