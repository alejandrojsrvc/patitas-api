...
export class MercadoPagoPaymentAdapter implements PaymentProvider {
  ...
  public async createPaymentLink(
    ...
    if (!response.ok || typeof body.init_point !== 'string')
      throw new Error('Mercado Pago no pudo crear la preferencia.');
    return {
      provider: this.name,
      preferenceId: typeof body.id === 'string' ? body.id : undefined,
      paymentUrl: body.init_point,
  ...
  private verifySignature(
    ...
    const dataId = scalarString(data.id)?.toLowerCase() ?? '';
