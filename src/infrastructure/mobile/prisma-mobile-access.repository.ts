...
export class PrismaMobileAccessRepository implements MobileAccessRepository {
  ...
  public async record(input: RecordMobileAccessInput): Promise<void> {
    const now = new Date();
    const accessDate = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
    const deviceIdHash = createHash('sha256').update(input.deviceId).digest('hex');
    const unique = {
