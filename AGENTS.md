...
API NestJS organizada como monolito modular. Prisma administra exclusivamente las tablas propias sobre PostgreSQL/Neon; Auth usa JWT propio y Storage usa adapters S3 compatibles con R2 y MinIO.
...
- Auth utiliza Argon2id, JWT EdDSA y refresh tokens rotativos persistidos. Storage usa credenciales S3 limitadas a los buckets requeridos.
...
- `pnpm db:migrate -- --name <cambio>` solo se ejecuta contra PostgreSQL local.
- Producción solo ejecuta `pnpm db:deploy` desde CI/CD.
- No utilizar `prisma db push` ni cambios manuales del schema para tablas propias.
...
- Auth depende exclusivamente de `IdentityProvider`; ningún guard, controller o caso de uso puede implementar criptografía o persistencia concreta.
